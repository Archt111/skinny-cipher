#include <stdint.h>
#include <stdio.h>
#include "skinny.h"

/**
 * SKINNY-128-384 block cipher encryption.
 * Under 48-byte tweakey at k, encrypt 16-byte plaintext at p and store the 16-byte output at c.
 */
void skinny(unsigned char *c, const unsigned char *p, const unsigned char *k) {
    // it gets more complicated if I made the array in 4x4 form so 1x16 it is.
    unsigned char IS[16];
    unsigned char TK[48];
    for (unsigned int is = 0; is <  16; is++)
    {
        IS[i] = p + is;
    }  

    for (unsigned int ik = 0; ik < 48; ik++)
    {
        TK[i] = k + ik;
    }

   
    // one of 56 rounds of encryption    
    for (unsigned int r = 0; r < 56; r++)
    {   
        // printf("assigned");
        
        subCells(IS);     
        addConst(IS,r);      
        addRTkey(IS,TK);
        // printf("%02x", *(IS+2));      
        shiftRows(IS);        
        mixCols(IS);
    }
    
}


/* SKINNY Sbox */
const unsigned char S8[] = 
{

};

// constants of LFSR in AddConstant function
const unsigned char CONSTANTS[] = 
{
    0x01, 0x03, 0x07, 0x0f, 0x1f, 0x3e, 0x3d, 0x3b, 0x37, 0x2f, 0x1e, 0x3c, 0x39, 0x33, 0x27, 0x0e,
    0x1d, 0x3a, 0x35, 0x2b, 0x16, 0x2c, 0x18, 0x30, 0x21, 0x02, 0x05, 0x0b, 0x17, 0x2e, 0x1c, 0x38,
    0x31, 0x23, 0x06, 0x0d, 0x1b, 0x36, 0x2d, 0x1a, 0x34, 0x29, 0x12, 0x24, 0x08, 0x11, 0x22, 0x04,
    0x09, 0x13, 0x26, 0x0c, 0x19, 0x32, 0x25, 0x0a
};


// Give the byte that only contains desired bit at the end
void single_bit(unsigned char* bit , unsigned char* byte, unsigned int n)
{  
    *bit = (*byte >> n) & 0x01;
}


void subCells(unsigned char* initState)
{   
    for (unsigned int cell = 0; cell < 16; cell++)
        { 
            initState[cell] = S8[initState[cell]];
        }
}

void addConst(unsigned char* initState, unsigned int round)
{
    unsigned char c0 = CONSTANTS[round] & 0x0f;
    initState[0] ^= c0;

    unsigned char c1 = CONSTANTS[round] & 0x03;
    initState[4] ^= c1;

    unsigned char c2 = 0x02; 
    initState[8] ^= c2; 
}

void permutation(unsigned char* original, const unsigned int* permu)
{
    unsigned char copy[16];
    for (unsigned int i = 0; i < 16; i++)
        { 
            copy[i] = original[permu[i]];
            original[i] = copy[i];
        }
}

const unsigned int TK_PERMU[] = {9,15,8,13,10,14,12,11,0,1,2,3,4,5,6,7};
void LFSR_TK(unsigned char* tk, unsigned int i)
{
    unsigned char* byte;
    // remember to initialize var before putting them into function
    unsigned char xorb1[8];
    unsigned char xorb2[8];
    
    // applying only on the first two rows of TK2 or TK3
    for (unsigned int row = 0; row < 2; row++)
    {
        for (unsigned int col = 0; col < 4; col++)
        {
            // Why does this workk but tk[4*row + col]
            byte = tk + (4*row + col);

            if ( i == 2)
            {
                single_bit(&xorb1, byte, 5);
                single_bit(&xorb2, byte, 7);
                *byte = (*byte << 1) | (*xorb1 ^ *xorb2);                
            }
            else if (i == 3)
            {
                single_bit(&xorb1, byte, 0);
                single_bit(&xorb2, byte, 6);
                *byte = (*byte >> 1) | ((*xorb1 ^ *xorb2) << 7); 
            }
        }
    }
    for (unsigned int i = 0; i < 8; i++)
    {
        tk[i] = byte[i];
    }
}
void addRTkey(unsigned char* initState, unsigned char* tk)
{
    // first and second rows of all tweakey arrays are extracted and bitwise 
    // exclusive-ored to the cipher internal state, respecting the array positioning
    unsigned char* tk1 = tk;
    unsigned char* tk2 = tk + 16; 
    unsigned char* tk3 = tk + 32;

    for (unsigned int i = 0; i < 2; i++)
    {
        for (unsigned int j = 0; j < 4; j++)
        {
            unsigned char pos = 4*i + j;
            initState[pos] ^= tk1[pos] ^ tk2[pos] ^ tk3[pos];
        }
    }

    // permute all tweakey arrays
    permutation(tk1, TK_PERMU);
    permutation(tk2, TK_PERMU);
    permutation(tk3, TK_PERMU);
   
    // LFSR on TK2, TK3
    LFSR_TK(tk2,2);    
    LFSR_TK(tk3,3);
}

const unsigned int SR_PERMU[] = {0,1,2,3,7,4,5,6,10,11,8,9,13,14,15,12};
void shiftRows(unsigned char* initState)
{
    //printf("%02x", *(initState+2));
    permutation(initState, SR_PERMU);
}

const unsigned int M[] = {1,0,1,1,1,0,0,0,0,1,1,0,1,0,1,0};
void mixCols(unsigned char* initState)
{
    // Copy the original matrix so I don't mess with it
    unsigned char copy[16];
    for (unsigned int i = 0; i < 16; i++)
    {
        copy[i] = initState[i];
    }

    // first row of encrypted = [cell1 cell2 cell3 cell4] = [row0*col0 row0*col1 ...]
    for (unsigned int rowM = 0; rowM < 4; rowM++)
    {
        for (unsigned int col = 0; col < 4; col++)
        {
            unsigned char cryptCell = 0x00;
            for (unsigned int n = 0; n < 4; n++)
            {
                unsigned int posM = 4*rowM + n;
                unsigned int pos = 4*n + 4*rowM + col;
                cryptCell ^= M[posM] * copy[pos];
            }
            initState[4*rowM + col] = cryptCell;
        }
    }
}